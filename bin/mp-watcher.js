...
program
  ...
  .action(({ platform, tabWidth, updateIgnore }) => {
    ...
    require('../dist/index').watch({ platform, tabWidth, updateIgnore });
  });
...
program
  ...
  .action(({ platform, tabWidth }) => {
    tabWidth = toNumber(tabWidth)
    require('../dist/index').updateJson({ platform, tabWidth });
  });
...
program
  ...
  .action(({ platform, tabWidth }) => {
    tabWidth = toNumber(tabWidth)
    require('../dist/index').updateIgnore({ platform, tabWidth });
  });
...
program
  ...
  .action(({ platform, tabWidth, name, path }) => {
    tabWidth = toNumber(tabWidth)
    require('../dist/index').generateCollectionPage({ platform, tabWidth, name, path });
  });
